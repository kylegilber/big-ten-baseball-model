...
levels(NEB_H$PlayResult)[levels(NEB_H$PlayResult) == "SIngle"] <- "Single"
levels(NEB_T$PlayResult)[levels(NEB_T$PlayResult) == "SIngle"] <- "Single"
levels(NEB_B$PlayResult)[levels(NEB_B$PlayResult) == "SIngle"] <- "Single"
levels(NEB_P$PlayResult)[levels(NEB_P$PlayResult) == "SIngle"] <- "Single"
```
...
teams <- list("ILL_ILL", "IU", "IOW_HAW", "MAR_TER", "MIC_WOL", "MIC_SPA", 
  "MIN_GOP", "NEB", "NOR_CAT", "OSU_BUC", "PEN_NIT", "PUR_BOI", "RUT_SCA")
...
data25$`Win%` <- c(0.6, 0.522, 0.826, 0.3, 0.5, 0.45, 0.35, 0.4, 0.4, 0.15, 
  0.522, 0.3, 0.4)
```
...
vars <- data.frame(
  Variable = c("Win%","FIP","K9","BB9","WHIP","wOBA","K%","BB%","ISO"),
  Type = c("Response", rep("Predictor", 8)),
  Explanation = c(
...
indices <- sample(size, size = 0.7 * size)
train <- data24[indices,]
test <- data24[-indices,]

model <- glm(`Win%` ~ FIP_mean + K9_mean + BB9_mean + WHIP_mean + wOBA_mean + 
  `K%_mean` + `BB%_mean` + ISO_mean, data = train) %>% 
  stats::step(model, direction = "backward", trace = FALSE)
