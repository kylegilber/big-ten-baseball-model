...
tempname <- function(data) {
  ...
  # Perform frequency analysis for factors
  counts <- map(factors, ~ data %>% 
    count(GameID, Pitcher, .data[[.x]]) %>% 
    pivot_wider(names_from = .data[[.x]], values_from = n, values_fill = 0))
  
  # Derive innings-pitched pitcher statistic
  IP <- data %>% distinct(GameID, Inning, Pitcher) %>% 
    count(GameID, Pitcher, name = "IP")
  
  # Join on game and pitcher
  left_join(IP, reduce(counts, left_join, by = c("GameID", "Pitcher")),
    by = c("GameID", "Pitcher")) %>% 
    mutate(
