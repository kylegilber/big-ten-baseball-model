...
summarise_batter <- function(data, name) {
  ...
  reduce(counts, left_join, by = "Batter") %>% 
    ...
    mutate(
      ...
      # Derive year
      if (format(as.Date(Date), "%Y") == 2021) {rownum <- 1}
      else if (format(as.Date(Date), "%Y") == 2022) {rownum <- 2}
      else if (format(as.Date(Date), "%Y") == 2023) {rownum <- 3}
      else if (format(as.Date(Date), "%Y") == 2024) {rownum <- 4}
      else {rownum <- 5}
        
      # Weighted on-base average (wOBA)
      wOBA = (
        w[rownum,2] * Walk + 
        w[rownum,3] * HitByPitch + 
        w[rownum,4] * Single +
        w[rownum,5] * Double + 
        w[rownum,6] * tryCatch({Triple}, error = function(cond) {0}) +
        w[rownum,7] * HomeRun) / 
...
indices <- sample(size, size = 0.8 * size)
train <- data[indices,]
test <- data[-indices,]

model <- glm(`Win%` ~ FIP_mean + K9_mean + BB9_mean + WHIP_mean + wOBA_mean + 
  `K%_mean` + `BB%_mean` + ISO_mean + RBI + RA + Year, data = train)
