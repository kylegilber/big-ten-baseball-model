...
tempname <- function(data) {
  ...
  IP <- data %>% distinct(GameID, Inning, Pitcher, PitcherTeam) %>% 
    count(GameID, Pitcher, PitcherTeam, name = "IP")
  
  # Derive plate appearances batting statistic
  PA <- data %>% distinct(GameID, Batter, Inning, PAofInning) %>% 
    count(GameID, Batter, name = "PA")
  
  # Join on gameID and pitcher
  left_join(IP, reduce(pitcher.counts, left_join, by = c("GameID", "Pitcher")),
    by = c("GameID", "Pitcher")) %>% 
    mutate(
